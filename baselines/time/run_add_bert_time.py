...
def predict(predict_dir, output_dir, model_name, ratio):
    # load the Huggingface config, tokenizer, and model
    ratio = float(ratio)
    config = AutoConfig.from_pretrained(model_name)
    model_name_1 = "clulab/roberta-timex-semeval"
    config_1 = AutoConfig.from_pretrained(model_name_1)
    ...
    model_1 = AutoModelForTokenClassification.from_pretrained(model_name_1,
                                                              config=config_1)
    ...
    predictions_1, _, _ = trainer_1.predict(dataset)
    # write the predictions in Anafora XML format
    write_anafora(output_dir, dataset, ratio * predictions + (1 - ratio) * predictions_1, tokenizer, config)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--model", metavar="DIR", dest="model_name",
                        help="The model name.")
    parser.add_argument("-r", "--ratio", metavar="RAT", dest="ratio",
                        help="The ratio")
    args = parser.parse_args()
    predict(args.predict_dir, args.output_dir, args.model_name, args.ratio)
